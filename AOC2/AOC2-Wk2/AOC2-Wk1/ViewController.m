...
////////// ON CLICK ACTIONS / RUN / PUSHUP / SIT UP / SECONDVIEW //////////////////////////////////////////////////////////////////////////////////
    ...
    if (viewController !=nil)
    {
        if (button.tag == 3)
        {
        ...
        else if (button.tag == 0)
            {
                ...
                situpButton.enabled = TRUE;
                textfield.text = @"You ran 2 miles";
            }else if (button.tag == 1)
                {
                    runButton.enabled = TRUE;
                    pushupButton.enabled = FALSE;
                    situpButton.enabled = TRUE;
                    textfield.text = @"You did 25 reps";
                }else if (button.tag == 2)
                    {
                        runButton.enabled = TRUE;
                        pushupButton.enabled = TRUE;
                        situpButton.enabled = FALSE;
                        textfield.text = @"You did 50 reps";
                    }
...
-(IBAction)onStep:(id)sender
{
    ...
    if (stepper != nil) {
        int currentValue = stepper.value;
        
        label.text = [NSString stringWithFormat:@"Add reps or miles: %d", currentValue];
    }
